...
const EXTRACTED_DATA = `${API_BASE_PATH}/extracted_data/`; 
...
export async function uploadFileToBackend(file: File): Promise<{ success: boolean; message: string; data?: any }> {
	...
	console.log(`Tentando UPLOAD POST para: ${EXTRACTED_DATA} com arquivo: ${file.name}`);
	...
	try {
		const response = await fetch(EXTRACTED_DATA, {
			method: 'POST',
